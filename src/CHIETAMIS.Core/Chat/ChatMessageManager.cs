...
using CHIETAMIS.Friendships;
using CHIETAMIS.Friendships.Cache;
using CHIETAMIS.Chat;

namespace CHIETAMIS.Chat
{
    [AbpAuthorize]
    public class ChatMessageManager : AbpServiceBase, IChatMessageManager
    {
        ...
        private async Task HandleSenderToReceiverAsync(UserIdentifier senderIdentifier, UserIdentifier receiverIdentifier, string message, Guid sharedMessageId)
        {
            ...
            if (friendshipState == null)
            {
                ...
                var receiverUser = _userManager.GetUserById(receiverIdentifier.UserId);
        ...
        private async Task HandleReceiverToSenderAsync(UserIdentifier senderIdentifier, UserIdentifier receiverIdentifier, string message, Guid sharedMessageId)
        {
            ...
            if (friendshipState == null)
            {
                ...
                var senderUser = _userManager.GetUserById(senderIdentifier.UserId);
                await _friendshipManager.CreateFriendshipAsync(
                    new Friendship(
                        receiverIdentifier,
                        senderIdentifier,
            ...
            else if (GetUnreadMessageCount(senderIdentifier, receiverIdentifier) == 1)
            {
                ...
                await _userEmailer.TryToSendChatMessageMail(
                      _userManager.GetUserById(receiverIdentifier.UserId),
                      _userManager.GetUserById(senderIdentifier.UserId).UserName,
                      senderTenancyName,
                      sentMessage
                  );
            }
