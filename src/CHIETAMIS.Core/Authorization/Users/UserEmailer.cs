    ...
    public class UserEmailer : AbpServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        private async Task ReplaceBodyWithAttachmentAndSend(string emailAddress, string subject, StringBuilder emailTemplate, StringBuilder mailMessage)
        {
            ...
            try
            {
                ...
                msg.Attachments.Add(new Attachment(_webHostEnvironment.WebRootPath + "\\CHIETA Office Move.pdf"));
        ...
        public async Task DGSendSubmissionEmailAsync(string EmailAddress, string SDLNumber, string OrganisationName, string ProjectCode, string ProjectName)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear Grant Applicant/SDF, " + "<br />" + "<br />");
            mailMessage.AppendLine("Your application for Discretionary Grants (2024/2025 Cycle 1) Funding has been received by CHIETA." + "<br />" + "<br />");
            mailMessage.AppendLine("This application is for: " + "<br />" + "<br />");
            mailMessage.AppendLine("<b>" + "SDL Number: " + "</b>"  + SDLNumber + "<br />");
            mailMessage.AppendLine("<b>" + "Organisation Name: " + "</b>" + OrganisationName + "<br />");
            mailMessage.AppendLine("<b>" + "Project: " + "</b>" + ProjectName + "<br />" + "<br />");
            mailMessage.AppendLine("The next step is the evaluation of the application which will commence as soon as the Grant window is closed. Feedback will be communicated by the 31 May 2024.");
            ...
            mailMessage.AppendLine("Regards, " + "<br />");
            mailMessage.AppendLine("Grants and Projects. " + "<br />");
            await ReplaceBodyAndSend(EmailAddress, "Grant Application Submission Confirmation", null, mailMessage);
        }
        public async Task SendGenericEmailAsync(string EmailAddress, string Subject, string Salut ,string Body, string Signature)
        {
            ...
            var salut = Salut.Split("|");
            var messages = Body.Split("|");
            var footer = Signature.Split("|");

            var salutlen = salut.Length;
            var msglen = messages.Length;
            var footlen = footer.Length;

            if (msglen > 0 && footlen > 0 && salutlen > 0) {

                var mailHeader = new StringBuilder();
                var mailBody = new StringBuilder();
                var mailFooter = new StringBuilder();

                var mailMessage = new StringBuilder();

                for (var i = 0; i < salutlen; i++)
                {
                    mailHeader.AppendLine(salut[i]);
                }
                mailHeader.AppendLine("<br><br>");

                for (var i = 0; i < msglen; i++)
                {
                    mailBody.AppendLine(messages[i]);
                }
                mailBody.AppendLine("<br><br>");

                for (var i = 0; i < footlen; i++)
                {
                    mailFooter.AppendLine(footer[i]);
                }

                mailMessage.AppendLine(mailHeader.ToString());
                mailMessage.AppendLine(mailBody.ToString());
                mailMessage.AppendLine(mailFooter.ToString());

                await ReplaceBodyAndSend(EmailAddress, Subject, null, mailMessage);
            }
        }
        ...
        public async Task DGSendBulkOutcomeEmailsAsync(string EmailAddress)
        {
            ...
            mailMessage.AppendLine("Kindly note that your feedback for learning/Strategic Projects for the DG 2023/2024 Cycle 1 is available on the CHIETA IMS Portal (http://ims.chieta.org.za). Please login using your credentials which you used during the application phase to access the portal." + "<br />" + "<br />");
            mailMessage.AppendLine("Yours Sincerely," + "<br />");
            mailMessage.AppendLine("CEO O.B.O CHIETA duly appointed Executive Authority");
            await ReplaceBodyAndSend(EmailAddress, "Grant Application Outcome - DG2024/25 Cycle 1", null, mailMessage);
        }
        ...
        public async Task NotificationEmailsAsync(string EmailAddress)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear Valued Stakeholder, " + "<br />" + "<br />");
            mailMessage.AppendLine("Please find attached the CHIETA communication regarding the SIU Investigation for you Information." + "<br />" + "<br />");
            mailMessage.AppendLine("Regards," + "<br />" + "<br />");
            await ReplaceBodyWithAttachmentAndSend(EmailAddress, "CHIETA SIU Investigation", null, mailMessage);
        }

        public async Task OfficeMoveEmailsAsync(string EmailAddress)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear Valued Stakeholder, " + "<br />" + "<br />");
            mailMessage.AppendLine("CHIETA has relocated its Head Office. Please find attached the details of the new Head Office." + "<br />" + "<br />");
            mailMessage.AppendLine("Regards," + "<br />" + "<br />");
            await ReplaceBodyWithAttachmentAndSend(EmailAddress, "CHIETA Head Office Move", null, mailMessage);
        }

        public async Task MGSendSubmissionEmailAsync(string EmailAddress, string SDLNumber, string OrganisationName, string ProjectCode, string ProjectName)
        {
            ...
            mailMessage.AppendLine("Your application for Mandatory Grants Funding has been received by CHIETA." + "<br />" + "<br />");
            mailMessage.AppendLine("This application is for: " + "<br />" + "<br />");
            mailMessage.AppendLine("<b>" + "SDL Number: " + "</b>" + SDLNumber + "<br />");
            mailMessage.AppendLine("<b>" + "Organisation Name: " + "</b>" + OrganisationName + "<br />");
            mailMessage.AppendLine("<b>" + "Funding : " + "</b>" + ProjectName + "<br />" + "<br />");
            mailMessage.AppendLine("The next step is the evaluation of the application which will commence as soon as the Application window is closed. Further communication will follow.");
            mailMessage.AppendLine("Please contact your CHIETA Regional Office for any further assistance." + "<br />" + "<br />");
            mailMessage.AppendLine("Regards, " + "<br />");
            mailMessage.AppendLine("Grants Team. " + "<br />");
            await ReplaceBodyAndSend(EmailAddress, "Mandatory Application Submission Confirmation", null, mailMessage);
        }

        public async Task MGSendNotYetSubmittedAsync(string EmailAddress, string SDL_Number, string Organisation_Name)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear SDF, " + "<br />" + "<br />");
            mailMessage.AppendLine("This is reminder that the Mandatory Grant application window closes tonight at midnight." + "<br />" + "<br />");
            mailMessage.AppendLine("This application is for: " + "<br />" + "<br />");
            mailMessage.AppendLine("<b>" + "SDL Number: " + "</b>" + SDL_Number + "<br />");
            mailMessage.AppendLine("<b>" + "Organisation Name: " + "</b>" + Organisation_Name + "<br />" + "<br />");
            mailMessage.AppendLine("Please contact your CHIETA Regional Office for any further assistance." + "<br />" + "<br />");
            mailMessage.AppendLine("Regards, " + "<br />");
            mailMessage.AppendLine("Grants Team. " + "<br />");
            await ReplaceBodyAndSend(EmailAddress, "Mandatory Application Submission Reminder", null, mailMessage);
        }
        ...
        public async Task SendMandatoryRejectionEmailsAsync(string EmailAddress, string Reason)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear Valued Stakeholder, " + "<br />" + "<br />");
            mailMessage.AppendLine("During the Evaluation of your Mandatory Grant application the following issues were identified for your attention: " + "<br />" + "<br />");
            mailMessage.AppendLine("Comment: " + Reason + "<br />" + "<br />");
            mailMessage.AppendLine("Kindly Please kindly assist to provide the requested information for the application to be reviewed further." + "<br />" + "<br />");
            mailMessage.AppendLine("You can contact your Regional Skills Advisor for more details." + "<br />" + "<br />");
            mailMessage.AppendLine("Yours Sincerely," + "<br />");
            mailMessage.AppendLine("CHIETA Grants Team");
            await ReplaceBodyAndSend(EmailAddress, "MOA/Delivery Plan Update", null, mailMessage);
        }

        public async Task SendMandatoryExtensionEmailsAsync(string EmailAddress, string SDL_No, string Organisation_Name)
        {
            ...
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("Dear Valued Stakeholder, " + "<br />" + "<br />");
            mailMessage.AppendLine("Your request for an Extension for the Mandatory Grant Application has been reviewed. The outcome of the review is as follows: " + "<br />" + "<br />");
            mailMessage.AppendLine("SDL Number: " + SDL_No + "<br />" + "<br />");
            mailMessage.AppendLine("Organisation Name: " + Organisation_Name + "<br />" + "<br />");
            mailMessage.AppendLine("Extension Request Outcome:   Approved" + "<br />" + "<br />");
            mailMessage.AppendLine("Kindly proceed to complete you application on the IMS Platform (http://localhost:4200)." + "<br />" + "<br />");
            mailMessage.AppendLine("Please note that the deadline for the applications to be finalised is 31 May 2025." + "<br />" + "<br />");
            mailMessage.AppendLine("Yours Sincerely," + "<br />");
            mailMessage.AppendLine("CHIETA Grants Team");
            await ReplaceBodyAndSend(EmailAddress, "Mandatory Grant Extensions Request Outcome", null, mailMessage);
        }
    }
}
