...
namespace CHIETAMIS.Migrator
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            using (var bootstrapper = AbpBootstrapper.Create<CHIETAMISMigratorModule>())
            {
                bootstrapper.IocManager.IocContainer
                    .AddFacility<LoggingFacility>(
                        f => f.UseAbpLog4Net().WithConfig(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "log4net.config"))
                    );
